...
`}},{"../utils":8}],7:[function(j,k){const q=j("./utils"),v=q.is,w=q.type,x=q.sameType;k.exports=function(){function E(aa,ba,ca){var da=Y.pages[aa],ea=$("[page="+aa+"]");return ea?void(Y.activePage=ea,ba&&!v.object(ba)&&X("Page [%s] nav params must be an object!",aa),da&&da.nav&&(v.function(da.nav)?da.nav(ba):X("Page [%s] nav property must be a function!",aa)),$("[page].active").removeClass("active"),Y.activePage.addClass("active"),$(document).trigger("rasti-nav"),ca||(da&&da.url?v.string(da.url)?window.history.pushState(aa,null,"#"+da.url):V("Page [%s] url property must be a string!",aa):window.history.pushState(aa,null))):X("Page [%s] container not found",aa)}function F(aa,ba,ca){var da=Y.templates[aa];if(!da)return X("Template [%s] is not defined",aa);var ea=$("[template="+aa+"]");if(!ea.length)return X("No element bound to template [%s]. Please bind one via [template] attribute.",aa);var fa=ea[0];if(!ba){var ga=ea.attr("data");if(!ga)return X("No data found for template [%s]. Please provide in ajax response or via [data] attribute in element:",aa,fa);if(ba=Y.data[ga],!ba)return X("Undefined data source \"%s\" in [data] attribute of element:",ga,fa)}if(!ba.length)return ea.html("<div msg center textc>NO RESULTS</div>");var ha=ea.attr("paging");if(ha?M(ea,ba):ea.html(da(ba)),fa.hasAttribute("stats")){var ia=$("<div section class=\"stats\">");ia.html(app.options.stats.replace("%n",ba.length).replace("%t",ca)),ea.prepend(ia)}var ja=ea.attr("fx");if(ja){var ka=Y.fx[ja];if(!ka)return X("Undefined fx \"%s\" in [fx] attribute of element",ja,fa);ha?ka(ea.find(".results")):ka(ea)}}function G(aa){var ba=aa.split(" ")[0],ca=Y.themes[ba];if(!ca)return X("Theme [%s] not found",ba);var da=aa.split(" ")[1]||v.object(ca.maps)&&Object.keys(ca.maps)[0]||"dark",ea=v.object(ca.maps)&&ca.maps[da]||Y.themeMaps[da];if(!ea)return X("Theme map [%s] not found",da);V("Setting theme [%s:%s]",ba,da),Y.activeTheme=ca;var ga,ha,ia,ja,ka,fa={font:ca.font||Y.themes.base.font};for(var la of Object.keys(ea)){if(!Y.themeMaps.dark[la])return X("Mapping error in theme [%s]. Incorrect theme map property [%s]",ba,la);for(var ma in ga=[ia,ja]=ea[la].split(" "),ha=[ca.palette[ia],ca.palette[ja]],ha)ka=Y.themeMaps.dark[la].split(" ")[ma],ka&&!ha[ma]&&(ha[ma]=Y.themes.base.palette[ga[ma]],!ha[ma]&&(W("Mapping error in theme [%s] for attribute [%s]. Color [%s] not found. Falling back to default color [%s].",ba,la,ga[ma],ka),ha[ma]=Y.themes.base.palette[ka]));fa[la]=ha}$("style[theme]").html(R(fa));for(var na of Object.keys(ca.palette)){var oa=ca.palette[na];$("."+na).css("background-color",oa)}}function H(aa){var ba=Y.langs[aa];if(!ba)return X("Lang [%s] not found",aa);if(!v.object(ba))return X("Lang [%s] must be an object!",aa);V("Setting lang [%s]",aa),Y.activeLang=aa;var da,ea,fa,ca=$(),ga="label header text placeholder".split(" ");ga.forEach(function(ha){ca=ca.add("["+ha+"]")}),ca.each(function(ha,ia){for(var ja in ia.hasAttribute("fixed")&&(ia=ia.children[0]),da=$(ia),ea=ia.langkeys,ea||(ea={},ga.forEach(function(ka){da.attr(ka)&&(ea[ka]=da.attr(ka))}),ia.langkeys=ea),ea)fa=S(aa,ea[ja]),"text"==ja?da.text(fa||ea[ja]):fa?da.attr(ja,fa):null})}function I(aa,ba){function ca(ka){if(ha.html(fa.template(ka,aa)),Z){var la=aa.attr("field"),ma=aa.closest("[page]").attr("page");W("Detected %s invalid data entries for field [%s] in page [%s]",Z,la,ma),Z=0}}var da=aa[0],ea="SELECT"==da.nodeName?"select":aa.attr("rasti");if(!ea)return X("Missing block type, please provide via [rasti] attribute in element:",da);var fa=Y.blocks[ea];if(!fa)return X("Undefined block type \"%s\" in [rasti] attribute of element:",ea,da);if(!ba){var ga=aa.attr("data");if(!ga)return X("Missing datakey, please provide via [data] attribute in element:",da);if(ba=Y.data[ga],!ba)return X("Undefined data source \"%s\" in [data] attribute of element:",ga,da)}var ha,ia;if("multi"===ea){var ia=aa.attr("field");if(!ia)return X("Missing [field] attribute value in element:",da);ha=aa.closest("[page]").find("[options="+ia+"]"),ha.length||(ha=$("<div field rasti="+ea+" options="+ia+">"),aa.closest("[page]").children(".page-options").append(ha))}else ha=aa;v.function(ba)?ba(ca):ca(ba)}function K(aa){function ba(ja){return Y.activePage.find(ja).length||Y.activePage.attr("page")===ja.attr("page")}var ga,ha,ia,ca=aa[0],da=aa.children(":not(.page-options)"),ea=$("<div class=\"tab-labels\">"),fa=$("<div class=\"bar\">");da.each(function(ja,ka){ga=$(ka),ga.attr("tab",ja),ha=ga.attr("label")||ga.attr("header")||ga.attr("name")||"TAB "+(ja+1),ea.append($(`<div tab-label=${ja} text="${ha}">`))}),ea.append(fa).appendTo(aa),ea.on("click",function(ja){var ka=$(ja.target),la=ka.attr("tab-label"),ma=da.filter(`[tab="${la}"]`);da.removeClass("active"),ma.addClass("active")[0].scrollIntoView(),ea.children().removeClass("active"),ka.addClass("active")}),aa.on("scroll",function(){ia=ca.scrollLeft/ca.scrollWidth,fa.css({left:ia*ca.offsetWidth})}),$(document).on("rasti-nav",function(){ba(aa)&&(fa.css({width:ca.offsetWidth/da.length}),!ea.children(".active").length&&ea.children().first().click())}),$(window).on("resize",function(){ba(aa)&&(ea.find(".active").click(),fa.css({width:ca.offsetWidth/da.length}))})}function L(aa){var ba=aa[0],ca="SELECT"==ba.nodeName?"select":aa.attr("rasti");if(!ca)return X("Missing block type, please provide via [rasti] attribute in element:",ba);var da=Y.blocks[ca];return da?void(aa.attr("data")&&I(aa),da.init(aa)):X("Undefined block type \"%s\" in [rasti] attribute of element:",ca,ba)}function M(aa,ba){function ca(la){$results.html(ea(la)),$controls.find(".page").html(ha.page+"/"+ha.total),v.function(fa)&&fa($results)}var ia,ja,ka,da=aa.attr("template"),ea=Y.templates[da],fa=aa.attr("fx")&&Y.fx[aa.attr("fx")],ga=parseInt(aa.attr("paging")),ha=O(da,ba,ga);aa[0].hasAttribute("columns")&&(ja=`<div class="columns floatl ib_"><label>Columns:</label><button btn>1</button><button btn value=2>2</button><button btn value=3>3</button></div>`),1<ha.total&&(ia=`<div class="paging ib_"><button class="btn prev">&lt;</button><span class="page"></span><button class="btn next">&gt;</button></div>`),ka=`<div class="sizes floatr ib_"><label>Page size:</label><button btn value=5>5</button><button btn value=10>10</button><button btn value=20>20</button></div>`,aa.html(`<div class="results scrolly"></div><div class="controls small bottom centerx ib_">${ja||""}${ia||""}${ka}</div>`),$controls=aa.children(".controls"),$results=aa.children(".results"),$controls.on("click",".next",function(){ca(ha.next())}),$controls.on("click",".prev",function(){ca(ha.prev())}),$controls.on("click",".sizes button",function(){ha.setPageSize(this.value),ca(ha.next()),1<ha.total?$controls.find(".paging").show():$controls.find(".paging").hide()}),$controls.on("click",".columns button",function(){$results.removeClass("columns-2 columns-3").addClass(this.value?"columns-"+this.value:"")}),$results.html(ea(ha.next())),$controls.find(".page").html(ha.page+"/"+ha.total)}function O(aa,ba,ca){let da=new _(aa,ba,ca);return Y.pagers.set(aa,da),da}function Q(aa,ba){var ca=Y.ajax[aa];if(!v.function(ca))return X("Ajax method ["+aa+"] is not defined");var da=$("[ajax="+aa+"]");if(!da.length)return X("No container element bound to ajax method [%s]. Please bind one via [ajax] attribute",aa);var fa,ea={};da.find("[field]").each(function(ga,ha){$el=$(ha),fa=$el.attr("field"),fa&&(ea[fa]=$el.val()||$el.attr("value"))}),ca(ea,ba)}function R(aa){return`body {font: ${aa.font};color: ${aa.text[0]};}[page]    { background-color: ${aa.page[0]}; }[panel]   { background-color: ${aa.panel[0]}; }[section] { background-color: ${aa.section[0]}; }
...
[header]:before { color: ${aa.header[0]}; }[label]:not([header]):before  { color: ${aa.label[0]}; }`}function S(aa,ba){if(!v.object(Y.langs[aa]))return void X("Lang [%s] is not defined",aa);var ca=Y.langs[aa][ba];return v.string(ca)?ca:void W("Lang [%s] does not contain key [%s]",aa,ba)}function T(aa){var ba=$(`<div fixed label="${aa.attr("label")}" >`);aa.wrap(ba),aa[0].removeAttribute("label")}function V(...aa){Y.options.log&&console.log.call(this,...aa)}function W(...aa){Y.options.log&&console.warn.call(this,...aa)}function X(...aa){Y.options.log&&console.error.call(this,...aa)}var Y=this,Z=0;this.activePage=null,this.activeTheme="",this.activeLang="",this.pagers=new Map,this.options={root:"",lang:"",log:!1,theme:"base",stats:"%n results in %t seconds"},this.pages={},this.data={},this.ajax={},this.utils={is:v,type:w,sameType:x},this.templates={},this.langs={},this.themes={base:{font:"normal 14px sans-serif",palette:{white:"#fff",lighter:"#ddd",light:"#bbb",mid:"#888",dark:"#444",darker:"#222",black:"#000",detail:"darkcyan",lighten:"rgba(255,255,255,0.1)",darken:"rgba(0,0,0,0.1)"}}},this.themeMaps={dark:{page:"darker lighten",panel:"dark lighten",section:"mid lighten",field:"light darker",btn:"detail darker",header:"darker",label:"darker",text:"darker"},light:{page:"lighter darken",panel:"mid lighten",section:"light darken",field:"lighter dark",btn:"detail light",header:"dark",label:"mid",text:"dark"}},this.fx={stack:function(aa){aa.children().each(function(ba,ca){setTimeout(function(){ca.style.opacity=1,ca.style.marginTop="15px"},50*ba)})}},this.blocks=j("./blocks/all");class _{constructor(aa,ba,ca){return(this.id=aa,!v.string(aa))?X("Pager id must be a string"):(this.logid=`Pager for template [${this.id}]:`,v.array(ba)?v.number(ca)?void(this.results=ba,this.page_size=ca,this.page=0,this.total=Math.ceil(this.results.length/this.page_size)):X("%s Page size must be a number",this.logid):X("%s Results must be an array",this.logid))}next(){return this.hasNext()?this.page++:W("%s No next page",this.logid),this.getPageResults(this.page)}prev(){return this.hasPrev()?this.page--:W("%s No previous page",this.logid),this.getPageResults(this.page)}hasNext(){return this.results.length>this.page*this.page_size}hasPrev(){return 1<this.page}setPageSize(aa){return aa=parseInt(aa),v.number(aa)?void(this.page_size=aa,this.page=0,this.total=Math.ceil(this.results.length/this.page_size)):X("%s Must specify a number as the page size",this.logid)}getPageResults(aa){if(!v.number(aa))return X("%s Must specify a page number to get results from",this.logid),[];try{var ba=(aa-1)*this.page_size;return this.results.slice(ba,ba+this.page_size)}catch(ca){return X("%s Could not get results of page %s, error:",this.logid,aa,ca),[]}}}return function(){Array.prototype.remove=function(aa){var ba=this.indexOf(aa);0<=ba&&this.splice(ba,1)},String.prototype.capitalize=function(){return this.length&&this[0].toUpperCase()+this.slice(1).toLowerCase()}}(),{activePage:function(){return Y.activePage},activeLang:function(){return Y.activeLang},activeTheme:function(){return Y.activeTheme},pages:this.pages,data:this.data,ajax:this.ajax,utils:this.utils,langs:this.langs,themes:this.themes,themeMaps:this.themeMaps,blocks:this.blocks,templates:this.templates,fx:this.fx,options:this.options,config:function(aa){for(var ba in Y)"object"===$.type(Y[ba])&&"object"===$.type(aa[ba])&&Object.assign(Y[ba],aa[ba])},init:function(aa){v.object(aa)&&Object.keys(Y.options).forEach(function(ha){x(Y.options[ha],aa[ha])&&(Y.options[ha]=aa[ha])}),Y.options.lang||(keys=Object.keys(Y.langs),keys.length&&(Y.options.lang=keys[0]));var ba="<style blocks>";for(var ca in Y.blocks)ba+=Y.blocks[ca].style;ba+="</style>",$("body").append(ba),$("body").append("<style theme>"),$("[page]").each(function(ha,ia){$(ia).append("<div class=\"page-options\">")}),$("[rasti]").each(function(ha,ia){L($(ia))}),$("select[data]").each(function(ha,ia){I($(ia))}),$("[tabs]").each(function(ha,ia){K($(ia))}),$("[nav]").click(function(){var ia=$(this),ja=ia.attr("nav"),ka={};if(!ja)return X("Please provide a page name in [nav] attribute of element:",el);if(this.hasAttribute("params")){var la=Y.activePage,ma=ia.attr("params");ma?(ma=ma.split(" "),ma.forEach(function(na){ka[na]=la.find("[navparam="+na+"]").val()})):la.find("[navparam]").each(function(na,oa){return ia=$(oa),ca=ia.attr("navparam"),ca?void(ka[ca]=ia.val()):X("Please provide a param name in [navparam] attribute of element:",oa)})}E(ja,ka)}),$("[submit]").click(function(){var oa,ia=$(this),ja=ia.attr("submit"),ka=ia.attr("then"),la=ia.attr("render"),ma=ka&&v.function(Y.utils[ka]),na=window.performance.now();return ja?void(ka&&!ma&&X("Utility method [%s] provided in [then] attribute is not defined",ka),ia.addClass("loading").attr("disabled",!0),Q(ja,function(pa){oa=window.performance.now();var qa=Math.floor(oa-na)/1e3;V("Ajax method [%s] took %s seconds",ja,qa),ma&&Y.utils[ka](pa),la&&F(la,pa,qa),ia.removeClass("loading").removeAttr("disabled")})):X("Plase provide an ajax method in [submit] attribute")}),$("[render]").not("[submit]").click(function(){var ia=$(this),ja=ia.attr("render");return ja?void F(ja):X("Please provide a template name in [render] attribute of element:",el)});for(var da of"click change".split(" "))$("["+da+"]").each(function(ha,ia){var ja=$(ia),ka=ja.attr(da);return app.utils[ka]?void $(this).on(da,app.utils[ka]):X("Undefined utility method \"%s\" declared in [%s] attribute of element:",ka,da,ia)});var ea,fa;for(var ga in Y.pages){if(ea=Y.pages[ga],!v.object(ea))return X("Page [%s] must be an object!",ga);if(fa=$("[page="+ga+"]"),!fa.length)return X("No container element bound to page [%s]. Please bind one via [page] attribute",ga);if(ea.init){if(!v.function(ea.init))return X("Page [%s] init property must be a function!",ga);V("Initializing page [%s]",ga),Y.activePage=fa,ea.init()}}Y.options.lang&&!Y.activeLang&&H(Y.options.lang),Y.options.lang||$("[text]").each(function(ha,ia){$(ia).text($(ia).attr("text"))}),"input select textarea".split(" ").forEach(function(ha){$(ha+"[label]").each(function(ia,ja){T($(ja))})}),Y.activeTheme||G(Y.options.theme),window.onpopstate=function(ha){var ia=ha.state||location.hash.substring(1);ia?ha.state?E(ia,null,!0):E(ia):E(Y.options.root)};var ea=location.hash.substring(1)||Y.options.root;E(ea&&Y.pages[ea]?ea:$("[page]").first().attr("page")),$(document).trigger("rasti-ready")},get:function(aa){if(!Y.activePage||!Y.activePage.length)return X("Cannot get(%s), active page is not defined",aa);var ba=Y.activePage.find("["+aa+"]");return ba.length||X("Cannot get(%s), element not found in page [%s]",aa,Y.activePage.attr("page")),ba},set:function(aa,ba){if(!Y.activePage||!Y.activePage.length)return X("Cannot set(%s), active page is not defined",aa);var ca=Y.activePage.find("["+aa+"]");ca.length||X("Cannot set(%s), element not found in page [%s]",aa,Y.activePage.attr("page")),ca.each(function(da,ea){ea.value=ba,$(ea).change()})},add:function(aa,...ba){if(!Y.activePage||!Y.activePage.length)return X("Cannot add(%s), active page is not defined",aa);var ca=Y.activePage.find("["+aa+"]");ca.length||X("Cannot add(%s), element not found in page [%s]",aa,Y.activePage.attr("page")),ca.each(function(da,ea){ba.forEach(function(fa){v.array(fa)?ea.value=ea.value.concat(fa):ea.value.push(fa)}),$(ea).change()})},navTo:E,render:F,setLang:H,setTheme:G,updateBlock:I,toggleFullScreen:function(){var ba="moz webkit".split(" ");ba.forEach(function(ca){ca+"FullscreenElement"in document&&(document[ca+"FullscreenElement"]?document[ca+"CancelFullScreen"]&&document[ca+"CancelFullScreen"]():document.documentElement[ca+"RequestFullScreen"]())})}}}},{"./blocks/all":1,"./utils":8}],8:[function(j,k){function q(A){var B=Object.prototype.toString.call(A);return B.substring(8,B.length-1).toLowerCase()}const z={};"object function array string number regex boolean".split(" ").forEach(function(A){z[A]=function(B){return q(B)===A}}),k.exports={is:z,type:q,sameType:function(A,B){return q(A)===q(B)},checkData:function(A){switch(typeof A){case"string":A={value:A,label:A,alias:A.toLowerCase()};break;case"object":z.string(A.value)&&z.string(A.label)?z.string(A.alias)?A.alias=A.alias.toLowerCase()+" "+A.value.toLowerCase():(A.alias&&(error("Invalid data property \"alias\" (must be a string):",A),invalidData++),A.alias=A.value.toLowerCase()):(error("Invalid data object (must have string properties \"value\" and \"label\"):",A),invalidData++,A={value:"",label:"INVALID DATA",alias:""});break;default:error("Invalid data (must be a string or an object):",A),invalidData++,A={value:"",label:"INVALID DATA",alias:""};}return A},random:function(){return(0|6*Math.random())+1},onMobile:function(){return 500>window.innerWidth}}},{}]},{},[7])(7)});